...
class ReadPostTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function test_it_returns_post_loaded_from_storage()
    {
        ...
        $request = $this->mockRequest("api/v1/posts/$id");
    ...
    public function test_it_returns_pagination_from_storage()
    {
        /** @var PostStorageInterface|\PHPUnit_Framework_MockObject_MockObject $storage_mock */
        $storage_mock = $this->getMock(PostStorageInterface::class);
        $storage_mock->expects($this->any())
            ->method('getPostsBefore')
            ->willReturn($this->getMock(PaginatorInterface::class));

        $request = $this->mockRequest("api/v1/posts/");
        $response = new Response();

        $controller = new ReadPost($storage_mock);
        /** @var ModelResponse $returned_value */
        $returned_value = $controller($request, $response, ['id' => null]);
        $this->assertInstanceOf(ModelResponse::class, $returned_value);
        $this->assertInstanceOf(PaginatorInterface::class, $returned_value->getModel());
    }
}
